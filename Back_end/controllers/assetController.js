...
const createAsset = async (req, res) => {
  const { asset_name, asset_tag, serial_number, category_id, purchase_date, purchase_cost, supplier_id, warranty_expiry, asset_condition, status, department_id } = req.body;
  
  try {
    // Check if asset_tag already exists
    const tagExists = await pool.query("SELECT * FROM assets WHERE asset_tag = $1", [asset_tag]);
