...
const createDisposal = async (req, res) => {
  const { asset_id, disposal_date, disposal_method, reason, approved_by } = req.body;
...
const deleteDisposal = async (req, res) => {
  ...
  try {
    ...
    const disposalResult = await pool.query('SELECT asset_id FROM asset_disposals WHERE id = $1', [id]);
