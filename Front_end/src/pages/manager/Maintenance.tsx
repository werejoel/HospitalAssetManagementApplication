...
export default function Maintenance() {
  ...
  const analytics = useMemo(() => {
    const totalCost = maintenanceRecords.reduce(
      (sum: number, record: any) => sum + Number(record.cost || 0),
      0,
    );
    ...
    const inProgressRecords = maintenanceRecords.filter(
      (r: any) => r.status === "in_progress",
    ).length;

    const maintenanceTypes = maintenanceRecords.reduce(
      (acc: { [key: string]: number }, record: any) => {
  ...
  return (
    <div className="px-4 sm:px-6 md:p-6">
      <PageHeader
        ...
        description="Track all maintenance activities"
      >
          ...
          <TableBody>
            {filteredRecords.map((r: any) => {
              ...
              const technician = users.find(
                (u: any) => u.user_id === r.technician_id,
              );
              const getRowColor = () => {
                switch (r.status) {
                  case "completed":
                  ...
                  case "in_progress":
                    return "bg-blue-50/30 border-l-4 border-l-blue-400";
                  case "scheduled":
                    return "bg-amber-50/30 border-l-4 border-l-amber-400";
                  case "cancelled":
                    return "bg-gray-50/30 border-l-4 border-l-gray-400";
                  default:
                    return "";
                }
              };
                ...
                <TableRow key={r.id} className={getRowColor()}>
                  ...
                  <TableCell>
                    <div className="flex items-center gap-2">
                      ...
                      {r.maintenance_type === "predictive" && (
                        <TrendingUp className="w-4 h-4 text-purple-500" />
                      )}
                      {r.maintenance_type === "emergency" && (
          ...
          <form onSubmit={handleSaveMaintenance} className="space-y-4">
            ...
            <div>
              ...
              >
                <SelectTrigger
                  className={
                    formErrors.maintenance_type ? "border-red-500" : ""
                  }
                >
            ...
            <div>
              ...
              <Select
                ...
                onValueChange={(value) =>
                  setMaintenanceForm({ ...maintenanceForm, status: value })
                }
              >
                <SelectTrigger>
