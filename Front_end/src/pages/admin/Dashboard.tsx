...
const Dashboard = () => {
  ...
  //statusCounts for dashboardStats
  const dashboardStats = useMemo(() => {
    ...
    const underMaintenance = filteredAssets.filter((asset) => {
    ...
    const disposed = filteredAssets.filter(
      (asset) => normalizeStatus(asset.status) === "disposed",
    ).length;
  ...
  const topValueAssets = useMemo(
    () =>
      filteredAssets
        .sort(
          (a, b) => Number(b.purchase_cost || 0) - Number(a.purchase_cost || 0),
        )
        .slice(0, 5),
    [filteredAssets],
  );
  ...
  const handleExport = () => {
    const csv = [
      ...
      ...filteredAssets.map((a) => [
        ...
        a.status,
        a.purchase_cost || "N/A",
        departments.find((d) => d.id === a.department_id)?.department_name ||
          "N/A",
    ...
    <div className="dashboard-page">
      ...
      <div className="pt-4 px-4 sm:px-6 md:p-6">
              ...
              <p className="text-xs text-muted-foreground">
                ...
                {filteredDepartments.length} departments,{" "}
                {
                  filteredCategories.filter((c) =>
                    filteredAssets.some((a) => a.category_id === c.id),
                  ).length
                }{" "}
                active categories
              </p>
        ...
        <div className="dashboard-grid dashboard-stats-grid">
          <StatCard
            title="Total Value"
            value={`UGX ${(dashboardStats.totalValue / 1000000).toFixed(1)}M`}
            icon={DollarSign}
          />
          <StatCard
            title="Departments"
        ...
        <div className="dashboard-grid dashboard-charts-grid">
                  ...
                  >
                    ...
                    <div className="text-right">
                      <p className="text-sm font-bold">
                        UGX{" "}
                        {(Number(asset.purchase_cost || 0) / 1000000).toFixed(
                          1,
                        )}
                        M
                      </p>
          ...
          {/* Status Distribution */}
          <div className="dashboard-card">
            <h3 className="dashboard-card-heading">
        ...
        <div className="dashboard-card">
          ...
          {filteredFaults.length === 0 ? (
            <div className="text-center py-8 text-muted-foreground">
              {search
                ? "No fault reports found matching your search."
